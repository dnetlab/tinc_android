...
{
	struct sockaddr_in *network_part = (struct sockaddr_in *)ip;;
	struct sockaddr_in *mask_in = (struct sockaddr_in *)mask;
	int network_part_len = 0;

	network_part->sin_addr.s_addr = network_part->sin_addr.s_addr & mask_in->sin_addr.s_addr;
	printf("mask = %08x\n", mask_in->sin_addr.s_addr);
	int i;
	for(i = 0; i < 32; i++)
	{
		unsigned long bit = 1 << i;
		if (mask_in->sin_addr.s_addr & bit)
		{
			network_part_len++;
		}
	}

	inet_ntop(AF_INET, &network_part->sin_addr, ret_buf, 20);
...
int net_tool_get_if_subnet(char *if_name, char *buf)
{
	...
	if (ret == 0)
	{
		convert_subnet_to_string(&ip_addr, &mask_addr, buf);
	}
	return ret;
}
...
char *net_tool_http_client2(int method, char *host, int port, char *uri, char *body, int body_len, int *recv_len)
{
	char *ret = NULL;
	char *web_head;
	char head_buf[2000];
	...
	if (method == 0)
	{
		...
		total_len = head_len + 1;
		body = NULL;
	}
	else
	{
		web_head = "POST %s HTTP/1.1\r\n"
		"Host: %s\r\n"
		"User-Agent: Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:35.0) Gecko/20100101 Firefox/35.0\r\n"
		"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
		"Accept-Language: en-US,en;q=0.5\r\n"
